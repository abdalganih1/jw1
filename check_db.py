...
# Parse the URL
# mysql+pymysql://root:@localhost:3306/jewelry_db
...
try:
    ...
    with conn.cursor() as cursor:
        cursor.execute("SELECT id, name, jeweler_id, price FROM products")
        ...
        pc = cursor.fetchall()
        print(f"Product Categories associations: {len(pc)}")
        for item in pc:
            print(item)
finally:
    if 'conn' in locals():
